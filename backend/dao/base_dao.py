...
class BaseDAO:
    ...
    Subclasses only set `table`, `pk` and `model`; the column list is read
    automatically from the model's dataclass fields, so each concrete DAO
    keeps just its own search/JOIN queries."""
    ...
    def _cols(self):
        # All columns except the primary key (auto-generated by the DB).
        return [f.name for f in fields(self.model) if f.name != self.pk]

    def _to_model(self, row):
        # SELECT * keeps the table column order, which matches the field order.
